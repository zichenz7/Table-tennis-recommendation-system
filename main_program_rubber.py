...
def create_radar_chart(df, rubber_name, categories):
    ...
    plt.title(f'Performance Analysis for {rubber_name}')
...
def get_user_preferences():
    criteria = ["Speed", "Spin", "Control", "Weight", "Durable", "Price"]
...
# Main program
print("\nWelcome to the Table Tennis Rubber Recommendation System!")
print("Please answer the following questions based on your preferences, and we will recommend the best rubber for you.")
...
ranking_df = pd.DataFrame({
    ...
    'Rubber Name': alternatives[ranked_indices],
    'Score': scores[ranked_indices].round(3)
})

# Save complete results
ranking_df.to_csv('rubber_ranking_results.csv', index=False)

# Display top 10 results and radar charts
print("\nBased on your preferences, here are the top 10 recommended rubbers:")
print("=" * 60)
print(ranking_df.head(10).to_string(index=False))
print("=" * 60)

print("\nGenerating performance analysis charts for recommended rubbers...")
for i in range(10):
    rubber_name = ranking_df.iloc[i]['Rubber Name']
    fig = create_radar_chart(df_normalized, rubber_name, categories)
    plt.show()
    plt.close(fig)

print("\nThe complete ranking results have been saved to rubber_ranking_results.csv")
print("Thank you for using the Table Tennis Rubber Recommendation System!")
